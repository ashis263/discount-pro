...
const BrandsByCategory = () => {
    ...
    const handleCategoryClick = (category) => {
        if (category === 'all') {
            ...
            let filtered;
            if(coupons){
                filtered = coupons.filter(coupon => coupon.category === category);
            }
            setBrands(filtered);
            setActive(category);
        }
    ...
    return (
        <div className="space-y-5 py-5 sm:py-10">
            ...
            <div className="grid grid-cols-1 gap-2 w-[37%]">
                ...
                {
                    categories && categories.map(category => <button onClick={() => handleCategoryClick(category)}  className={active === category ? "btn btn-sm hover:bg-primary bg-primary text-white" : "btn btn-outline text-primary btn-sm hover:bg-primary hover:border-primary"} key={category}>{category}</button>)
                }
            </div>
            <div className="w-4/5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-5">
                {
                    brands && brands.map(brand => <div onClick={() => navigate(`/brand/${brand.id}`)} className="bg-white rounded-xl shadow-primary shadow p-2 mb-3" key={brand.id}>
