...
const TopBrands = () => {
    ...
    let tops;
    if(coupons){
        tops = coupons.filter(coupon => coupon.rating > 4);
    }
    return (
        <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 py-5 sm:py-10 bg-gray-100 rounded-xl">
                {
                    tops && tops.map(top => <div onClick={() => navigate(`/brand/${top.id}`)} className="bg-white rounded-xl shadow-md p-5 place-self-center border border-dotted w-40 sm:w-44 lg:w-48 space-y-2" key={top.id}>
