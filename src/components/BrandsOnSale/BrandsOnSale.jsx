...
const BrandsOnSale = () => {
    ...
    let onSales;
    if(coupons){
        onSales = coupons.filter(coupon => coupon.isSaleOn === true);
    }
    return (
        <aside className="py-5 sm:py-10 sm:w-1/3 lg:w-[25%] xl:w-1/5">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-1 gap-5 py-5 sm:py-10 bg-gray-100 rounded-xl">
                {
                    onSales && onSales.map(onSale => <div onClick={() => navigate(`/brand/${onSale.id}`)} className="bg-white rounded-xl shadow-md p-5 place-self-center border border-dotted w-40 sm:w-44 lg:w-48 space-y-2" key={onSale.id}>
